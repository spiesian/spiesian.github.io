...
for i in r:
    ...
    cur.execute("""
        ...
        """,
        (id, title, description, date_created, starting_bid, current_bid, active, image))
    conn.commit()
    id += 1
cur.execute("""
    INSERT INTO auctions_image (id, image, auction_id)
    SELECT id, image, id FROM auctions_auction;
    """)
conn.commit()
...
# extract data from GSA_Auctions API
...
def index(request):
    ...
    paginator = Paginator(auctions, 36)
